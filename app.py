...
# --- PHẦN 1: CSS FIX ---
# Thêm !important để đảm bảo trình duyệt luôn lật ngược video/ảnh bất kể thiết bị
st.markdown("""
    <style>
    .stRadio [data-testid="stMarkdownContainer"] p { font-size: 18px; font-weight: bold; }
    div[data-testid="stMetric"] { background-color: #ffffff; padding: 15px; border-radius: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    ...
    /* Lật ngược luồng webcam đang soi */
    video {
        transform: scaleX(-1) !important;
        -webkit-transform: scaleX(-1) !important;
    }

    /* Lật ngược ảnh xem trước ngay trong widget sau khi bấm chụp */
    [data-testid="stCameraInput"] img {
        transform: scaleX(-1) !important;
        -webkit-transform: scaleX(-1) !important;
    }
...
with col1:
    ...
    img_data = None
    if st.session_state.input_method == "upload":
        img_data = st.file_uploader("Kéo thả file...", type=["jpg", "png", "jpeg"])
    elif st.session_state.input_method == "camera":
        img_data = st.camera_input("Chụp ảnh để phân tích")
...
with col2:
    ...
    if img_data is not None:
        ...
        # --- PHẦN 2: LOGIC XỬ LÝ LẬT ẢNH ---
        # Nếu dùng camera (bất kể cam trước hay sau), ta thực hiện lật ảnh bằng code 
        # để kết quả hiển thị và kết quả AI khớp với cái mình thấy trên màn hình đã lật (CSS)
        if st.session_state.input_method == "camera":
            image = ImageOps.mirror(image)
            st.image(image, caption='Ảnh đã chụp (Đã lật ngược)', use_container_width=True)
        else:
            st.image(image, caption='Dữ liệu tải lên', use_container_width=True)
        ...
        if model is not None:
            ...
            st.markdown("---")
            # Giả sử prob < 0.5 là người (tùy thuộc vào cách bạn train model)
            if prob < 0.5:
                st.success(f"## ✅ KẾT LUẬN: ĐÂY LÀ NGƯỜI")
                st.balloons()
            else:
